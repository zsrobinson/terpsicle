...
| V4: Reviews | `v2/reviews-api`, `v2/reviews-ui`, `v2/reviews-publish` | `v2/reviews-api` done (#69); `v2/reviews-ui` in review |
...
- **Reviews' UI** (`v2/reviews-ui`, `src/features/reviews`): `/reviews` (find a course; your classes from this browser's plans, read with a raw IndexedDB read), `/reviews/courses/$code`, `/reviews/instructors/$id?course=`, `/reviews/mine` and `/reviews/policy`. The combined rating is `combineRatings` (`src/core/reviews/combine.ts`, landed here rather than in `v2/reviews-publish`); until the `reviews/` R2 family publishes our numbers, our side is read from `reviews/list` (at most 10 pages per instructor; a course page reads its current instructors' lists, at most 12). Writing, reporting and sign-in prompts open in place (no popovers or dialogs); deleting waits out an 8 s Undo toast, and a closing page sends it with `keepalive`. The header's account corner is a light link (Sign in, or your picture to Settings) so Reviews shares as little code as possible with `/schedule`: each shared module splits the scheduler's chunk, and gzip counts the pieces separately. Reviews pages have their own bundle budget (270 KB). `pnpm dev:mock` has `REVIEWS_ENABLED` on; its Worker has no PlanetTerp files, so new reviews there get minted instructor ids and, with no Workers AI, wait for a person.
- **Production builds no longer include the fixtures.** `createDataSource` imports them only in mock and test builds (as `?demo` already did), so no production chunk can reach them.
