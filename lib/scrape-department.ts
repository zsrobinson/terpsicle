...
export async function scrapeDepartment(dept: string) {
    ...
    courses.forEach(courseDiv => {
        ...
        const courseCreditsDiv = courseDiv.querySelector('span.course-max-credits') || 
                                  courseDiv.querySelector('span.course-min-credits');
        const courseCredits = parseFloat(courseCreditsDiv?.textContent?.trim() || '0');
        ...
        if (genEdGroupDiv) {
            ...
            const genEdSubcategories = genEdGroupDiv.querySelectorAll('span.course-subcategory');
            ...
            genEdSubcategories.forEach((subcategory, index) => {
                ...
                if (subcategory.nextSibling?.textContent?.includes('or') === false || 
