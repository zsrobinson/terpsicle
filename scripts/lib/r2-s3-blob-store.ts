...
async function credentials(): Promise<{
  ...
  const { R2_ACCESS_KEY_ID, R2_SECRET_ACCESS_KEY, CLOUDFLARE_API_TOKEN } =
    process.env;
  ...
  if (!CLOUDFLARE_API_TOKEN) {
    throw new Error(
      "Set R2_ACCESS_KEY_ID and R2_SECRET_ACCESS_KEY, or CLOUDFLARE_API_TOKEN, to write to R2",
    );
  }
  ...
  if (!response.ok || !id) {
    throw new Error(
      `CLOUDFLARE_API_TOKEN didn't verify (HTTP ${response.status})`,
    );
  }
  return {
    accessKeyId: id,
...
export async function createR2S3BlobStore(
  bucket = DATA_BUCKET,
): Promise<BlobStore> {
  ...
  const fail = async (what: string, response: Response): Promise<never> => {
    throw new Error(
      `R2 ${what} failed: HTTP ${response.status} ${(await response.text()).slice(0, 200)}`,
    );
  };

  const get = async (key: string) => {
    // Uncompressed on purpose: a gzipped response carries a weak etag
    // (W/"…"), which If-Match then never matches.
  ...
  return {
    ...
    async delete(key) {
      ...
      if (!response.ok && response.status !== 404)
        await fail(`DELETE ${key}`, response);
    },
    async list(prefix) {
      const keys: string[] = [];
