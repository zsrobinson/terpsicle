...
| `site` | `SCHEDULE_PATH` (`/schedule`, where share links, deep links and emails point); who skips the marketing page at `/`: `hasSessionCookie(cookieHeader)` (the cookie name lives here only), `hasSavedWork(plans, blocks)` (a course, a second plan in a term, or a block; self-contained for the head script) and `landingFor({signedIn, savedWork})` |
