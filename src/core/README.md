...
| `problems` | `planProblems({plan, index, blocks, travel, campus, seats, changes, pendingDepts?})` → `Problem[]` (sections in `pendingDepts`, not loaded yet, are never cancelled), sorted by severity, with `switch` or `accept-change` fixes; `countBySeverity`; `planWithSection` (what a switch fix produces) |
