...
const cmsc330 = aCourse({
  code: "CMSC330",
  sections: [
    aSection({ code: "0101", meetings: [aMeeting({ start: 600, end: 650, building: "IRB" })] }),
    aSection({ code: "0201", meetings: [aMeeting({ days: ["Tu", "Th"], start: 600, end: 675 })] }),
  ],
});
...
const cmsc351 = aCourse({
  code: "CMSC351",
  sections: [
    aSection({ code: "0101", meetings: [aMeeting({ start: 630, end: 680, building: "ESJ" })] }), // overlaps 330
    aSection({ code: "0201", meetings: [aMeeting({ start: 655, end: 705, building: "KEY" })] }), // 5 min after 330, far
    aSection({ code: "0301", meetings: [aMeeting({ start: 540, end: 595, building: "KEY" })] }), // 5 min before 330, far
    aSection({ code: "0401", meetings: [aMeeting({ start: 660, end: 710, building: "ESJ" })] }), // fits
    aSection({ code: "0501", meetings: [aMeeting({ start: 750, end: 800, building: "ESJ" })] }), // lunch block
    aSection({ code: "0601", delivery: "online-async", meetings: [anUntimedMeeting()] }), // no set times
    aSection({ code: "0701", cancelled: true }),
  ],
});

const english = aCourse({ code: "ENGL393", sections: [aSection({ meetings: [] })] });

function input(courses: Course[], plan = aPlan({ termId: TERM, courses: [placed(cmsc330, "0101")] })): FitInput {
  return {
    plan,
    index: buildCatalogIndex(TERM, courses),
    blocks: [aBlock({ termId: TERM, id: "block-lunch", label: "Lunch", start: 720, end: 780 })],
...
describe("fitLabel", () => {
  ...
  it.each([
    ["0101", { kind: "overlaps", with: { kind: "course", courseCode: "CMSC330" } }],
    ["0201", { kind: "not-enough-time", direction: "after", courseCode: "CMSC330" }],
    ["0301", { kind: "not-enough-time", direction: "before", courseCode: "CMSC330" }],
    ["0401", { kind: "fits" }],
    ["0501", { kind: "overlaps", with: { kind: "block", blockId: "block-lunch", label: "Lunch" } }],
  ...
  it("says in-plan for the placed section, and fits it against the rest", () => {
    ...
    // biome-ignore lint/style/noNonNullAssertion: the builder above has it
    expect(fitLabel(ctx, cmsc330, cmsc330.sections[1]!)).toEqual({ kind: "fits" });
  });

  it("never rules a section out for travel before routes load", () => {
    const noRoutes = buildFitContext({ ...input([cmsc330, cmsc351]), routes: null });
    expect(fitLabel(noRoutes, cmsc351, section("0201"))).toEqual({ kind: "fits" });
  });

  it("treats a saved-for-later course as not in the plan", () => {
    const plan = aPlan({ termId: TERM, courses: [saved("CMSC330"), placed(cmsc351, "0401")] });
    const c = buildFitContext(input([cmsc330, cmsc351], plan));
    // biome-ignore lint/style/noNonNullAssertion: the builder above has it
    expect(fitLabel(c, cmsc330, cmsc330.sections[0]!)).toEqual({ kind: "fits" });
  });

  it("allows tight connections", () => {
    const tight = aCourse({
      code: "MATH240",
      sections: [aSection({ meetings: [aMeeting({ start: 654, end: 700, building: "ESJ" })] })],
    });
  ...
  it("ignores overlaps across date ranges that don't meet", () => {
    ...
    const halfPlanned = aCourse({
      code: "CMSC330",
      sections: [
        aSection({ code: "0101", meetings: [aMeeting({ start: 600, end: 650 })], dates: { start: "2027-01-25", end: "2027-03-12" } } as never),
      ],
    });
    const c = buildFitContext(input([halfPlanned, halfCourse], firstHalf));
    // biome-ignore lint/style/noNonNullAssertion: one section
    expect(fitLabel(c, halfCourse, halfCourse.sections[0]!)).toEqual({ kind: "fits" });
  });

  it("checks exact minutes when times aren't on 5-minute slots", () => {
    const odd = aCourse({
      code: "MATH240",
      sections: [aSection({ meetings: [aMeeting({ start: 651, end: 698, building: "IRB" })] })],
    });
