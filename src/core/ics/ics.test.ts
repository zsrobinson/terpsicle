...
/** Spring 2027 as the provost publishes it: classes Jan 27–May 11, spring break Mar 14–21. */
...
const half = aCourse({
  ...
  sections: [
    aSection({
      ...
      dates: { start: "2027-03-22", end: "2027-04-30" },
    } as never),
  ],
});

function ref(course: Course, i = 0) {
  const section = course.sections[i];
  if (!section) throw new Error(`${course.code} has no section ${i}`);
  return sectionRef(course, section);
}
...
function ok(result: ReturnType<typeof buildIcs>) {
  if (result.kind !== "ok")
    throw new Error(`expected an ics, got ${result.kind}`);
  return result;
}
...
describe("buildIcs", () => {
  it("matches the golden file", async () => {
    ...
    await expect(result.ics).toMatchFileSnapshot(
      "__fixtures__/spring-2027.ics",
    );
  });
  ...
  it("moves the first event past a holiday on the first day", () => {
    const cal = aPublishedCalendar({
      noClasses: [{ name: "Snow day", start: "2027-01-27", end: "2027-01-27" }],
    });
    const all = lines(
      ok(buildIcs(input({ calendar: cal, sections: [ref(cmsc351)] }))).ics,
    );
    expect(all).toContain("DTSTART;TZID=America/New_York:20270129T100000");
    expect(
      all.some((l) => l.startsWith("EXDATE") && l.includes("20270127")),
    ).toBe(false);
  });
  ...
  it("escapes text and describes the section", () => {
    const all = lines(ok(buildIcs(input())).ics);
    expect(all).toContain(
      "SUMMARY:ENGL393 Technical Writing\\; Online\\, Sync",
    );
    ...
    expect(all).toContain(
      "DESCRIPTION:Section 0101 · Clyde Kruskal\\, Justin Wyss-Gallifent",
    );
    expect(all).toContain("DESCRIPTION:Section 0312 · Instructor TBA");
  });

  it("says when the calendar isn't published", () => {
    const notYet = anUnpublishedCalendar({ termId: SPRING });
    expect(buildIcs(input({ calendar: notYet }))).toEqual({
      kind: "not-published",
    });
    expect(buildIcs(input({ calendar: null }))).toEqual({
      kind: "not-published",
    });
  });
  ...
  it("has nothing to add when nothing meets at set times", () => {
    const outside = aCourse({
      ...
      sections: [
        aSection({
          dates: { start: "2027-06-01", end: "2027-07-01" },
        } as never),
      ],
    });
  ...
  it("keeps UIDs stable and distinct", () => {
    expect(eventUid(SPRING, "CMSC351-0101", 0)).toBe(
      eventUid(SPRING, "CMSC351-0101", 0),
    );
    expect(eventUid(SPRING, "CMSC351-0101", 0)).toMatch(
      /^[0-9a-f]{16}@terpsicle\.com$/,
    );
    const uids = new Set([
      eventUid(SPRING, "CMSC351-0101", 0),
    ...
    const uidLines = (ics: string) =>
      lines(ics).filter((l) => l.startsWith("UID:"));
...
describe("dates", () => {
  it("does calendar arithmetic", () => {
    ...
    expect(eachDate("2027-03-13", "2027-03-15")).toEqual([
      "2027-03-13",
      "2027-03-14",
      "2027-03-15",
    ]);
  ...
  it("follows US daylight saving for America/New_York", () => {
    ...
    expect(new Date(easternToUtc("2027-01-27", 600)).toISOString()).toBe(
      "2027-01-27T15:00:00.000Z",
    );
    expect(new Date(easternToUtc("2027-05-11", 600)).toISOString()).toBe(
