...
function partWords(p: MessagePart): string {
  switch (p.kind) {
    case "text":
      return p.text;
    case "course":
...
describe("travel problems", () => {
  ...
  it("groups not-enough-time across days and offers the best switch", () => {
    ...
    expect(summary(ps)).toEqual([
      [
        "error",
        "not-enough-time",
        "Not enough time to get from CMSC330 to CMSC351",
        "Switch CMSC351 to 0401",
      ],
    ]);
    const [p] = ps;
    ...
    expect(words(p?.detail ?? [])).toBe(
      "12 min to get there, 5 min between classes · M, W, F",
    );
    expect(p?.fix).toEqual({
      kind: "switch",
      sectionKey: "CMSC351-0401",
      label: "Switch CMSC351 to 0401",
    });
    for (const problem of ps)
  ...
  it("warns about tight connections", () => {
    const plan = aPlan({
      termId: TERM,
      courses: [placed(cmsc330, "0101"), placed(cmsc351, "0301")],
    });
    const [p] = planProblems(input([cmsc330, cmsc351], plan));
    expect(p).toMatchObject({
      severity: "warning",
      kind: "tight-connection",
      id: "tight-connection:CMSC330-0101>CMSC351-0301",
    });
    expect(words(p?.title ?? [])).toBe(
      "Tight connection from CMSC330 to CMSC351",
    );
  });
  ...
  it("stays quiet until routes load", () => {
    ...
    expect(
      planProblems(input([cmsc330, cmsc351], plan, { campus: EMPTY_CAMPUS })),
    ).toEqual([]);
  });
});
...
describe("overlaps", () => {
  const math = aCourse({
    ...
    sections: [
      aSection({
        code: "0101",
        meetings: [
          aMeeting({ start: 630, end: 680, building: "IRB" }),
          aMeeting({ days: ["Tu"], start: 600, end: 650, building: "IRB" }),
        ],
      }),
      aSection({
        code: "0201",
  ...
  it("lists an overlap calmly, with its times, and a fix", () => {
    const plan = aPlan({
      termId: TERM,
      courses: [placed(cmsc330, "0101"), placed(math, "0101")],
    });
    ...
    expect(summary(ps)).toEqual([
      [
        "warning",
        "overlap",
        "CMSC330 and MATH240 overlap",
        "Switch MATH240 to 0201",
      ],
    ]);
  ...
  it("offers no fix when every alternative creates a new problem", () => {
    ...
    const plan = aPlan({
      termId: TERM,
      courses: [placed(cmsc330, "0101"), placed(only, "0101")],
    });
...
describe("section problems", () => {
  const course = aCourse({
    ...
    sections: [
      ...
      aSection({
        code: "0401",
        meetings: [anUntimedMeeting({ online: false })],
      }),
  ...
  it("warns about full and low sections, with seat words", () => {
    const seats = {
      ...
      "CMSC351-0501": aSeatTuple({ open: 20, total: 30 }),
    };
    ...
    expect(words(noList[0]?.detail ?? [])).toBe(
  ...
  it("notes TBA instructors and sections with no set times", () => {
    ...
    expect(summary(planProblems(input([course], plan("0301"))))).toEqual([
      [
        "info",
        "no-set-times",
        "CMSC351 0301 is online with no set times",
        null,
      ],
    ]);
    expect(
      words(planProblems(input([course], plan("0401")))[0]?.title ?? []),
    ).toBe("CMSC351 0401 has no set times");
  });

  it("ignores saved-for-later courses", () => {
    const p = aPlan({ termId: TERM, courses: [aSavedCourse("CMSC351")] });
    expect(
      planProblems(
        input([course], p, {
          seats: { "CMSC351-0101": aSeatTuple({ open: 0 }) },
        }),
      ),
    ).toEqual([]);
  });
...
describe("catalog changes", () => {
  ...
  it("errors on a cancelled section and offers another", () => {
    const after = aCourse({
      code: "CMSC351",
      sections: [before.sections[1] ?? aSection()],
    });
    const ps = planProblems(input([after], plan));
    expect(summary(ps)).toEqual([
      ["error", "cancelled", "CMSC351 0101 was cancelled", "Switch to 0201"],
    ]);
    expect(words(ps[0]?.detail ?? [])).toBe(
  ...
  it("describes changed dates", () => {
    ...
    expect(p?.fix?.label).toBe("Keep new times");
    expect(words(p?.detail ?? [])).toBe(
      "Now meets Mar 22–May 11; was the whole term.",
    );
  });
  ...
  it("errors on a changed section and offers to keep the new times", () => {
    ...
    expect(p).toMatchObject({
      ...
      fix: {
        ...
        label: "Keep new times",
      },
    });
    expect(words(p?.detail ?? [])).toBe(
      "Now Tu, Th 660–735 in IRB 1116; was M, W, F 600–650 in IRB 0324.",
    );
  });
  ...
  it("describes instructor and delivery changes", () => {
    const after = aCourse({
      code: "CMSC351",
      sections: [
        aSection({ code: "0101", instructors: [], delivery: "blended" }),
      ],
    });
    const [changed] = planProblems(input([after], plan)).filter(
      (p) => p.kind === "changed",
    );
    expect(changed?.fix?.label).toBe("Keep changes");
    expect(words(changed?.detail ?? [])).toBe(
      "Instructor is now TBA; was Ada Brandt. Now blended; was in person.",
    );
  });
  ...
  it("describes online and unlisted meetings", () => {
    const after = aCourse({
      ...
      sections: [
        aSection({
          code: "0101",
          meetings: [
            aMeeting({ online: true, building: null, room: null }),
            anUntimedMeeting(),
          ],
        }),
      ],
    ...
    const [p] = planProblems(input([after], plan));
    expect(words(p?.detail ?? [])).toBe(
      "Now M, W, F 600–650 online; no set times; was M, W, F 600–650 in IRB 0324.",
    );
    const bare = aPlan({
      termId: TERM,
      courses: [
        {
          ...placed(before, "0101"),
          snapshot: { ...snapshotOf(aSection()), meetings: [] },
        },
      ],
    });
...
describe("ordering and counts", () => {
  ...
  it("counts by severity", () => {
    const course = aCourse({
      sections: [
        aSection({ instructors: [], restriction: "Restricted to majors." }),
      ],
    });
    const plan = aPlan({ termId: TERM, courses: [placed(course, "0101")] });
    expect(countBySeverity(planProblems(input([course], plan)))).toEqual({
      error: 0,
      warning: 1,
      info: 1,
    });
  ...
  it("switches a plan's section with a fresh snapshot", () => {
    const course = aCourse({
      sections: [
        aSection({ code: "0101" }),
        aSection({ code: "0201", instructors: ["A B"] }),
      ],
    });
    const plan = aPlan({
      termId: TERM,
      courses: [placed(course, "0101"), aSavedCourse("CMSC330")],
    });
