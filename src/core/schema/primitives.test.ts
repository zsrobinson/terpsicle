...
describe("section keys", () => {
  it("round-trips course and section codes", () => {
    ...
    expect(parseSectionKey(key)).toEqual({ courseCode: "CMSC351", sectionCode: "0101" });
  });

  it("handles suffixed courses and alphanumeric sections", () => {
    expect(parseSectionKey("CMSC389N-FC01")).toEqual({ courseCode: "CMSC389N", sectionCode: "FC01" });
  });

  it("rejects things that aren't section keys", () => {
    for (const bad of ["CMSC351", "CMSC351-", "cmsc351-0101", "CMSC351 0101", "CMSC351-0101-2"]) {
...
describe("primitives", () => {
  it("accepts every term month code and nothing else", () => {
    for (const ok of ["202701", "202605", "202608", "202612"]) expect(TermIdSchema.safeParse(ok).success).toBe(true);
    for (const bad of ["202702", "20270", "2027-01", "spring"]) expect(TermIdSchema.safeParse(bad).success).toBe(false);
  });
