...
import { ReviewSummaryResultSchema, SubscribeInputSchema, SubscribeResultSchema } from "./api";
import { AcademicCalendarSchema } from "./calendar";
import { BuildingsFileSchema, RouteGeometrySchema, RoutesIndexSchema } from "./geo";
import {
  changesKey,
  dataCachePolicy,
  deptChunkKey,
...
describe("geo", () => {
  it("accepts buildings, the routes index and a route geometry", () => {
    const buildings = {
      schemaVersion: 1,
      buildings: [{ code: "IRB", number: "432", name: "Iribe Center", lat: 38.9891, lng: -76.9365 }],
    };
    expect(BuildingsFileSchema.safeParse(buildings).success).toBe(true);
    expect(RoutesIndexSchema.safeParse({ buildings: ["CSI", "IRB"], modes: ["standard", "accessible"] }).success).toBe(
      true,
    );
    expect(RoutesIndexSchema.safeParse({ buildings: [], modes: ["accessible", "standard"] }).success).toBe(false);
    const route = {
      schemaVersion: 1,
      from: "IRB",
      to: "CSI",
    ...
    expect(RouteGeometrySchema.safeParse({ ...route, coordinates: [[-76.9365, 38.9891]] }).success).toBe(false);
  });
});
...
describe("planetterp", () => {
  ...
  it("accepts a department file", () => {
    const record = { counts: [10, 40, 12, 9, 20, 8, 5, 10, 3, 1, 2, 0, 6, 4, 1], semesters: 6, latestTermId: "202501" };
    const dept = {
      schemaVersion: 1,
      dept: "CMSC",
      instructors: {
    ...
    const short = { ...dept, courses: { CMSC351: { all: { ...record, counts: [1, 2, 3] }, byInstructor: {} } } };
    expect(PlanetTerpDeptSchema.safeParse(short).success).toBe(false);
  });
});

describe("academic calendar", () => {
  const base = { schemaVersion: 1, termId: TERM, source: "https://provost.umd.edu/calendar.md", fetchedAt: NOW };
  ...
  it("represents published and not-yet-published terms", () => {
    ...
    expect(AcademicCalendarSchema.safeParse({ ...base, status: "not-published" }).success).toBe(true);
    const backwards = { ...published, noClasses: [{ name: "Oops", start: "2027-03-21", end: "2027-03-14" }] };
...
describe("server fn contracts", () => {
  it("rejects unknown input keys", () => {
    const input = { email: "testudo@umd.edu", termId: TERM, sectionKey: "CMSC351-0101" };
    expect(SubscribeInputSchema.safeParse(input).success).toBe(true);
    expect(SubscribeInputSchema.safeParse({ ...input, admin: true }).success).toBe(false);
    expect(SubscribeInputSchema.safeParse({ ...input, email: "not an email" }).success).toBe(false);
  });

  it("covers every subscribe and summary outcome", () => {
    expect(SubscribeResultSchema.safeParse({ status: "already-watching" }).success).toBe(true);
    const sent = { status: "confirmation-sent", subscriptionId: "A".repeat(22), manageToken: "b".repeat(43) };
    expect(SubscribeResultSchema.safeParse(sent).success).toBe(true);
    expect(ReviewSummaryResultSchema.safeParse({ status: "unavailable", reason: "daily-limit" }).success).toBe(true);
  });
});
...
describe("R2 keys", () => {
  it("builds the documented layout", () => {
    ...
    expect(deptChunkKey(TERM, "CMSC", HASH)).toBe(`catalog/${TERM}/dept/CMSC.${HASH}.json`);
    ...
    expect(routeGeometryKey("IRB", "CSI", "standard")).toBe("geo/route/IRB-CSI-standard.json");
  });

  it("caches hashed files forever and polls manifests", () => {
    expect(dataCachePolicy(deptChunkKey(TERM, "CMSC", HASH))?.cacheControl).toContain("immutable");
    expect(dataCachePolicy(manifestKey(TERM))).toEqual({ cacheControl: "public, no-cache", edgeTtlSeconds: 60 });
