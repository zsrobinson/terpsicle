...
import { aBlock, aCourse, aPlan, aSection, placed, saved } from "../test-support/builders";
import {
  coursesFromShare,
  sharedViewPlan,
...
const typical: SharePayload = {
  v: 1,
  termId: TERM,
  name: "Plan A",
  sections: ["CMSC351-0101", "CMSC330-0201", "STAT400-0101", "ENGL393-0312", "ECON200-0104"],
  saved: ["MUSC130"],
  blocks: [{ label: "Lunch", days: ["M", "W", "F"], start: 720, end: 780 }],
  colors: { CMSC351: "blue", CMSC330: "green", STAT400: "amber", ENGL393: "violet", ECON200: "cyan", MUSC130: "pink" },
};
...
describe("share codec", () => {
  ...
  it("is well under the plain-JSON encoding", () => {
    const plain = toBase64Url(deflateSync(strToU8(JSON.stringify(typical)), { level: 9 }));
    expect(encodeShare(typical).length).toBeLessThan(plain.length * 0.6);
  });

  it("builds the URL", () => {
    expect(shareUrl("https://terpsicle.com/", typical)).toBe(`https://terpsicle.com/?plan=${encodeShare(typical)}`);
  });

  it("drops empty lists and colors for courses outside the plan", () => {
    const p: SharePayload = { v: 1, termId: TERM, sections: [], saved: [], blocks: [], colors: { CMSC351: "blue" } };
    expect(normalizeSharePayload(p)).toEqual({ v: 1, termId: TERM, sections: [] });
    expect(decodeShare(encodeShare(p))).toEqual({ ok: true, payload: { v: 1, termId: TERM, sections: [] } });
  });

  it("round-trips any valid payload (property)", () => {
    const course = fc.stringMatching(/^[A-Z]{4}\d{3}[A-Z]{0,2}$/);
    const days = fc.subarray([...DAYS] as Day[], { minLength: 1 });
    const block = fc
      .record({ label: fc.string({ minLength: 1, maxLength: 40 }), days, start: fc.integer({ min: 0, max: 1439 }), len: fc.integer({ min: 1, max: 600 }) })
      .map(({ len, ...b }) => ({ ...b, end: Math.min(1440, b.start + len) }));
    const payload = fc
      .record({
        courses: fc.uniqueArray(course, { maxLength: 30 }),
        split: fc.nat(),
        sectionCode: fc.stringMatching(/^[A-Z0-9]{3,6}$/),
        name: fc.option(fc.string({ minLength: 1, maxLength: 60 }), { nil: undefined }),
        blocks: fc.array(block, { maxLength: 5 }),
        colors: fc.array(fc.option(fc.constantFrom(...COURSE_COLORS), { nil: null }), { maxLength: 30 }),
      })
    ...
    fc.assert(
      fc.property(payload, (p) => {
        expect(decodeShare(encodeShare(p))).toEqual({ ok: true, payload: normalizeSharePayload(p) });
      }),
    );
  });

  describe("bad links get a typed error", () => {
    const wire = (value: unknown) => toBase64Url(deflateSync(strToU8(JSON.stringify(value))));
    it.each([
      ["empty", ""],
    ...
    it("from a newer version", () => {
      expect(decodeShare(wire([2, TERM]))).toEqual({
        ok: false,
        error: { kind: "newer-version", message: "This link was made by a newer version of Terpsicle. Reload to open it." },
      });
    });
  });
...
describe("plans and share payloads", () => {
  const cmsc351 = aCourse({ code: "CMSC351", sections: [aSection({ code: "0101" }), aSection({ code: "0201", cancelled: true })] });
  ...
  it("encodes a plan with its term's blocks and its courses' colors", () => {
    const plan = aPlan({ termId: TERM, courses: [placed(cmsc351, "0101"), saved("CMSC330")] });
    const blocks = [aBlock({ termId: TERM }), aBlock({ termId: "202608", id: "block-fall" })];
    const payload = sharePayloadFromPlan(plan, blocks, { CMSC351: "blue", MATH240: "pink" });
    ...
    expect(sharePayloadFromPlan(aPlan({ termId: TERM }), [], {})).toEqual({ v: 1, termId: TERM, name: "Plan A", sections: [] });
  });

  it("caps each list at 40", () => {
    const many = Array.from({ length: 45 }, (_, i) => aCourse({ code: `CMSC${100 + i}` }));
    const plan = aPlan({ termId: TERM, courses: [...many.map((c) => placed(c, "0101")), ...many.map((c) => saved(c.code.replace("CMSC", "MATH")))] });
  ...
  it("shows missing sections as cancelled in the shared view", () => {
    const payload: SharePayload = { v: 1, termId: TERM, sections: ["CMSC351-0101", "CMSC351H-0101"], saved: ["CMSC330"] };
    const plan = sharedViewPlan(payload, index, "shared-view", "2026-09-25T12:00:00.000Z");
    ...
    expect(diffPlanAgainstCatalog(plan, index).map((d) => [d.key, d.kind])).toEqual([["CMSC351H-0101", "cancelled"]]);
  });

  it("saves a copy with fresh snapshots, naming what it dropped", () => {
    const payload: SharePayload = { v: 1, termId: TERM, sections: ["CMSC351-0101", "CMSC351-0201", "MATH240-0101"], saved: ["CMSC330", "ENGL101"] };
    const { courses, dropped } = coursesFromShare(payload, index);
    expect(courses).toEqual([placed(cmsc351, "0101"), saved("CMSC330")]);
