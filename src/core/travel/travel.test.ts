...
import { aCourse, aMeeting, aSection } from "~/fixtures";
...
import { type CampusMap, campusMap, EMPTY_CAMPUS } from "./campus";
import { connectionsByDay, planConnections } from "./connections";
import {
  connectionToExplain,
  travelMath,
  VERDICT_WORDS,
...
/**
 * IRB ↔ ESJ 1320 ft standard, 1500 accessible; IRB ↔ KEY has no accessible
 * route; SHM is unknown to everything.
 */
function routesTable() {
  const feet: Record<string, number | "no-route"> = {
    ...
    "accessible:IRB>KEY": "no-route",
    "accessible:KEY>IRB": "no-route",
...
describe("routes binary", () => {
  it("round-trips distances per mode, with a 0 diagonal and unknown cells", () => {
    ...
    expect(routeDistance(table, "IRB", "KEY", "accessible")).toBe("no-route");
      ...
      fc.property(
        ...
        (buildings, cells) => {
          ...
          const cell = (mode: string, from: string, to: string) => {
            ...
            return (
              cells[((mode === "standard" ? 0 : 64) + i * 8 + j) % 128] ?? null
            );
          };
          const table = decodeRoutes(
            encodeRoutes({ buildings, distance: cell }),
          );
              ...
              for (const to of sorted) {
                ...
                expect(routeDistance(table, from, to, mode)).toBe(
                  typeof expected === "number"
                    ? Math.min(ROUTES_MAX_FEET, expected)
                    : expected,
                );
              }
        },
  ...
  it("decodes from a subarray at an unaligned offset", () => {
    ...
    expect(
      routeDistance(
        decodeRoutes(bytes.buffer as ArrayBuffer),
        "B1",
        "A1",
        "accessible",
      ),
  ...
  it("clamps, rounds and treats non-finite as unknown", () => {
    const table = decodeRoutes(
      encodeRoutes({
        buildings: ["AA", "BB", "CC"],
        distance: (_m, from, to) =>
          from === "AA" ? 1e9 : to === "AA" ? 10.6 : Number.NaN,
      }),
    );
  ...
  describe("rejects bad files", () => {
    ...
    const mutate = (
      f: (b: Uint8Array, v: DataView) => void,
      bytes = good.slice(),
    ) => {
    ...
    it("unsorted or invalid index", () => {
      ...
      expect(() =>
        decodeRoutes(
          index('{"buildings":["BB","AA"],"modes":["standard","accessible"]}'),
        ),
      ).toThrow(/sorted/);
      expect(() =>
        decodeRoutes(index('{"buildings":["BB","AA"],"modes":["x"]}')),
      ).toThrow(/invalid/);
    });
  });
});
...
describe("walking math", () => {
  it("is ceil(feet / (mph × 88)) + extra", () => {
    ...
    expect(
      walkMinutes(1320, { ...typical, pace: "faster", extraMinutes: 5 }),
    ).toBe(10);
...
const campus: CampusMap = campusMap(routesTable(), {
  offCampus: [
    { code: "SGB4", name: "Universities at Shady Grove, Building IV" },
  ],
});
...
describe("connections", () => {
  const ref = (
    code: string,
    meetings: ReturnType<typeof aMeeting>[],
    extra = {},
  ) => {
    const course = aCourse({
      code,
      sections: [aSection({ meetings, ...extra })],
    });
  ...
  it("links consecutive in-person meetings in different buildings", () => {
    const a = ref("CMSC351", [
      aMeeting({ days: ["M", "W"], start: 600, end: 650, building: "IRB" }),
    ]);
    const b = ref("CMSC330", [
      aMeeting({ days: ["M", "W"], start: 660, end: 710, building: "ESJ" }),
    ]);
    const cs = planConnections([b, a], typical, campus);
    ...
    expect(
      connectionsByDay(cs).map((d) => [d.day, d.connections.length]),
    ).toEqual([
      ["M", 1],
      ["W", 1],
    ]);
  ...
  it("uses the accessible matrix and extra minutes", () => {
    const a = ref("CMSC351", [
      aMeeting({ days: ["M"], end: 650, building: "IRB" }),
    ]);
    const b = ref("CMSC330", [
      aMeeting({ days: ["M"], start: 657, end: 710, building: "ESJ" }),
    ]);
    ...
    const [c] = planConnections([a, b], settings, campus);
  ...
  it("skips same-building pairs, online meetings and non-consecutive ones", () => {
    const a = ref("CMSC351", [
      aMeeting({ days: ["M"], start: 540, end: 590, building: "IRB" }),
    ]);
    const same = ref("CMSC330", [
      aMeeting({ days: ["M"], start: 600, end: 650, building: "IRB" }),
    ]);
    const online = ref("ENGL393", [
      aMeeting({
        days: ["M"],
        start: 655,
        end: 700,
    ...
    const far = ref("MATH240", [
      aMeeting({ days: ["M"], start: 720, end: 770, building: "ESJ" }),
    ]);
    const cs = planConnections([a, same, online, far], typical, campus);
    // IRB (same) → ESJ (far) is the only walk: the online class doesn't count.
    expect(cs.map((c) => `${c.from.sectionKey}>${c.to.sectionKey}`)).toEqual([
      "CMSC330-0101>MATH240-0101",
    ]);
  });

  it("has no connection into a class something overlaps", () => {
    const long = ref("CMSC351", [
      aMeeting({ days: ["M"], start: 540, end: 720, building: "IRB" }),
    ]);
    const inside = ref("CMSC330", [
      aMeeting({ days: ["M"], start: 600, end: 650, building: "ESJ" }),
    ]);
    const after = ref("MATH240", [
      aMeeting({ days: ["M"], start: 730, end: 780, building: "KEY" }),
    ]);
    const cs = planConnections([long, inside, after], typical, campus);
    // CMSC351 ends last before MATH240, so that's the walk; CMSC330 overlaps CMSC351.
    expect(cs.map((c) => `${c.from.sectionKey}>${c.to.sectionKey}`)).toEqual([
      "CMSC351-0101>MATH240-0101",
    ]);
  });
  ...
  it("links meetings within one section (lecture → discussion)", () => {
    ...
    const [c] = planConnections([s], typical, campus);
    expect(c).toMatchObject({
      from: { meetingIndex: 0 },
      to: { meetingIndex: 1 },
      walkMinutes: 10,
  ...
  it("ignores meetings whose dates don't intersect", () => {
    ...
    const second = ref(
      "CMSC330",
      [aMeeting({ days: ["M"], start: 660, end: 710, building: "ESJ" })],
      {
        dates: { start: "2027-03-22", end: "2027-05-10" },
      },
    );
    expect(planConnections([first, second], typical, campus)).toEqual([]);
  });
  ...
  it("links from each half-term class when two share an end time", () => {
    ...
    const h2 = ref(
      "CMSC330",
      [aMeeting({ days: ["M"], end: 650, building: "KEY" })],
      {
        dates: { start: "2027-03-22", end: "2027-05-10" },
      },
    );
    const full = ref("MATH240", [
      aMeeting({ days: ["M"], start: 660, end: 710, building: "ESJ" }),
    ]);
    const cs = planConnections([h1, h2, full], typical, campus);
    expect(cs.map((c) => c.from.sectionKey).sort()).toEqual([
      "CMSC330-0101",
      "CMSC351-0101",
    ]);
  });

  it("has no route where UMD's network can't route the pair", () => {
    const a = ref("CMSC351", [
      aMeeting({ days: ["M"], end: 650, building: "IRB" }),
    ]);
    const b = ref("CMSC330", [
      aMeeting({ days: ["M"], start: 660, end: 710, building: "KEY" }),
    ]);
    const [c] = planConnections(
      [a, b],
      { ...typical, accessible: true },
      campus,
    );
    expect(c).toMatchObject({
      verdict: "no-route",
    ...
    expect(verdictMessage(c)).toEqual([
      {
        kind: "text",
        text: "UMD's map has no accessible route for this connection.",
      },
    ]);
    expect(verdictMessage({ ...c, mode: "standard" })[0]).toEqual({
      kind: "text",
      text: "UMD's map has no route for this connection.",
    });
    expect(travelMath(c, typical)).toBeNull();
  });

  it("never connects to an off-campus meeting", () => {
    const a = ref("CMSC351", [
      aMeeting({ days: ["M"], end: 650, building: "IRB" }),
    ]);
    const shady = ref("BMGT110", [
      aMeeting({ days: ["M"], start: 660, end: 710, building: "SGB4" }),
    ]);
    const b = ref("CMSC330", [
      aMeeting({ days: ["M"], start: 720, end: 770, building: "ESJ" }),
    ]);
    const cs = planConnections([a, shady, b], typical, campus);
    expect(cs.map((c) => `${c.from.sectionKey}>${c.to.sectionKey}`)).toEqual([
      "CMSC351-0101>CMSC330-0101",
    ]);
    expect(campusMap(null, null)).toEqual(EMPTY_CAMPUS);
  });
  ...
  it("is unknown without routes or without a distance", () => {
    ...
    const b = ref("CMSC330", [
      aMeeting({ days: ["M"], start: 660, end: 710, building: "SHM" }),
    ]);
    for (const map of [EMPTY_CAMPUS, campus]) {
      const [c] = planConnections([a, b], typical, map);
      expect(c).toMatchObject({
        verdict: "unknown",
        distanceFeet: null,
        walkMinutes: null,
...
describe("explanations", () => {
  const pair = (gapStart: number) => {
    const a = aCourse({
      ...
      sections: [
        aSection({
          meetings: [aMeeting({ days: ["M"], end: 650, building: "IRB" })],
        }),
      ],
    });
    const b = aCourse({
    ...
    const refs = [a, b].map((course) => {
      // biome-ignore lint/style/noNonNullAssertion: one section each
      return sectionRef(course, course.sections[0]!);
    });
    const [c] = planConnections(refs, typical, campus);
    if (!c) throw new Error("expected a connection");
    return c;
  };
  ...
  it("says how late you'd be", () => {
    ...
    const late = pair(655);
    expect(verdictMessage(late)[0]).toEqual({
      kind: "text",
    ...
    expect(verdictMessage(pair(700))[0]).toEqual({
      kind: "text",
      text: "Plenty of time: ",
    });
    expect(
    ...
    ).toEqual([{ kind: "text", text: `${VERDICT_WORDS.unknown}.` }]);
  });
  ...
  it("shows the math for one real connection", () => {
    ...
    expect(
      travelMath({ ...c, distanceFeet: null, walkMinutes: null }, typical),
    ).toBeNull();
    const ok = pair(700);
